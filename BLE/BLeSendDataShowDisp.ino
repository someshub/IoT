...
class MyServerCallbacks : public BLEServerCallbacks {
  void onConnect(BLEServer* pServer) {
    ...
    displayUpdated = false;  // Flag to indicate display update required
...
class MyCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (value.length() > 0) {
      receivedText = String(value); // Convert std::string to String
      Serial.println("Received Value: " + receivedText);
      displayUpdated = true;  // Set flag to update display
    }
  }
};
...
void setup() {
  ...
  // Add the callback to handle writes
...
void loop() {
  if (deviceConnected) {
    ...
    if (displayUpdated) {
      ...
      displayUpdated = false; // Reset the flag after updating the display
    }
